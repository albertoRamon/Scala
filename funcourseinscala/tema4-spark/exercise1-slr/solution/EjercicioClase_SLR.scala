...
class EjercicioSLR(sc: SparkContext) extends Serializable {
   ...
   */
  val xs: RDD[Double] = points map (_._1)
  val ys: RDD[Double] = points map (_._2)
   ...
   */
  val sum1: Double = points map {
    case (x, y) => (x - meanX) * (y - meanY)
  } sum
  val sum2: Double = xs map (x => math.pow(x - meanX,2)) sum
